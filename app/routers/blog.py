...
@router.post("/posts", response_model=schemas.BlogPostResponse, dependencies=[Depends(get_current_admin)])
async def create_post(data: schemas.BlogPostCreate, db: AsyncSession = Depends(get_db)):
    ...
    post = models.BlogPost(
        title=data.title,
        slug=data.slug,
        excerpt=data.excerpt or data.content[:200],  # fallback
...
@router.patch("/posts/{post_id}", response_model=schemas.BlogPostResponse, dependencies=[Depends(get_current_admin)])
async def update_post(post_id: UUID, data: schemas.BlogPostUpdate, db: AsyncSession = Depends(get_db)):
    post = await db.get(models.BlogPost, post_id)
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
@router.post("/tags", response_model=schemas.BlogTagResponse, dependencies=[Depends(get_current_admin)])
...
@router.patch("/tags/{tag_id}", response_model=schemas.BlogTagResponse, dependencies=[Depends(get_current_admin)])
async def update_tag(tag_id: UUID, data: schemas.BlogTagUpdate, db: AsyncSession = Depends(get_db)):
...
async def upload_image(file: UploadFile = File(...), admin=Depends(get_current_admin)):
    ...
    url = await save_image(file, folder="blog")
    return {"url": url}
